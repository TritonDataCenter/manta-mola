...
function getJob(jobId, cb) {
        ...
        self.mantaClient.job(jobId, function (err, job) {
                if (err && err.name === 'ResourceNotFoundError') {
                        ...
                        console.log(jobId);
                        var path = '/' + self.mantaClient.user + '/jobs/' +
                                jobId + '/job.json';
                        getObject.call(self, path, function (err2, res) {
                                if (err2) {
...
JobManager.prototype.run = function run(cb) {
        ...
        //TODO: Add jobName and assets if they don't already exist in the job
        //      definition.
        vasync.pipeline({
                'funcs': [
...
JobManager.prototype.createMarlinJob = function createMarlinJob(opts, cb) {
        ...
        self.mantaClient.createJob(job, function (err, jobId) {
                ...
                //Add objects to job...
                self.mantaClient.addJobKey(jobId, objects,
                                           aopts, function (err2) {
                        if (err2) {
                                cb(err2);
                                return;
...
JobManager.prototype.auditJob = function auditJob(job) {
        var self = this;
